...
// ─── DATA ────────────────────────────────────────────────────────────────────
const NAV_LINKS = ['清潔卸妝', '化妝水凝膠', '安瓶精華', '乳霜修護', '面膜護理', '防曬底妝', '院線課程儀器'];
...
const PRODUCT_IMAGE_ZOOM_NAMES = new Set([
  '亮白光采霜', '完美潔面卸妝膏', '記憶抗皺眼霜', 'PHA溫和煥膚',
  '急救安瓶-水合複合', '平衡爽膚水', '急救安瓶-積雪草毛孔', '急救安瓶-維他命美白', '亮顏防曬BB霜',
  '乳酸菌亮白面膜', '乳酸菌亮白⾯膜', '保濕補水霜', '精萃凝膠', '氧氣泡泡',
]);

const PRODUCT_PRIMARY_IMAGE_OVERRIDES = {
  '平衡爽膚水': 1,
};

const PRODUCT_IMAGE_SCALE_OVERRIDES = {
  '亮白光采霜': 1.24,
  '完美潔面卸妝膏': 1.22,
};

function normalizeProductImageScale(value) {
  const scale = Number(value);
  return Number.isFinite(scale) && scale > 0 ? scale : null;
}

function getDefaultProductImageScale(product) {
  return PRODUCT_IMAGE_SCALE_OVERRIDES[product?.nameZh] || (PRODUCT_IMAGE_ZOOM_NAMES.has(product?.nameZh) ? 1.08 : 1);
}

const PRODUCT_AUTO_IMAGE_TARGETS = { list: 0.78, detail: 0.72 };
const PRODUCT_IMAGE_BOUNDS_CACHE = new Map();

function getImageAnalysisSrc(src) {
...
function analyzeTransparentImageBounds(src) {
...
function ProductAutoImage({ src, alt, product, mode, style }) {
...
function getProductImage(product, width = 600) {
...
function applyVariantToProduct(product, variant) {
...
function buildSalesStats(orders) {
  const byId = {};
  const byName = {};
...
function getProductSalesCount(product, salesStats) {
...
function getPopularProducts(products, salesStats, limit = DEFAULT_POPULAR_PRODUCT_LIMIT) {
...
function getPromoDisplayPrice(product, user, promotions) {
...
const SINOPAC_PAYMENT_API = 'https://pay.ecladotaiwan.com';
const SINOPAC_NOTIFY_API = 'https://www.ecladotaiwan.com/api/sinopac/notify';
...
function extractPaymentLink(response) {
...
function getPaymentToken(response) {
  if (!response || typeof response !== 'object') return '';
  const candidates = [
    response.PayToken,
...
function buildPaymentNotes(methodLabel, response) {
...
const JOURNAL_ARTICLES = [
  {
    ...
    readTime:'6 min read',
    img:'https://images.unsplash.com/photo-1620916566398-39f1143ab7be?w=900&q=85&fit=crop',
  },
  {
    category:'保養觀念',
    ...
    readTime:'5 min read',
    img:'https://images.unsplash.com/photo-1600185365483-26d7a4cc7519?w=900&q=85&fit=crop',
  },
  {
    category:'專業分享',
    ...
    readTime:'7 min read',
    img:'https://images.unsplash.com/photo-1570172619644-dfd03ed5d881?w=900&q=85&fit=crop',
  },
  {
    category:'清潔筆記',
    ...
    readTime:'4 min read',
    img:'https://images.unsplash.com/photo-1556228578-8c89e6adf883?w=900&q=85&fit=crop',
  },
  {
    category:'安瓶應用',
    ...
    readTime:'6 min read',
    img:'https://images.unsplash.com/photo-1615397349754-cfa2066a298e?w=900&q=85&fit=crop',
  },
  {
    category:'專業教育',
    ...
    readTime:'5 min read',
    img:'https://images.unsplash.com/photo-1556228453-efd6c1ff04f6?w=900&q=85&fit=crop',
  },
];
...
function NavItem({ item, scrolled, setPage, user }) {
  ...
  const col = scrolled ? 'var(--dark)' : 'rgba(255,255,255,0.88)';
  return (
    <div ref={ref} style={{ position:'relative' }}
      onMouseEnter={() => item.children && setOpen(true)}
      onMouseLeave={() => setOpen(false)}>
      <button onClick={() => item.label === '所有產品' ? go('所有產品') : item.children ? setOpen(o=>!o) : go()} style={{
        background:'none', border:'none', cursor:'pointer',
        fontFamily:'var(--font-body)', fontSize:13, fontWeight:400,
        letterSpacing:'0.06em', color:col, padding:'8px 18px',
        display:'flex', alignItems:'center', gap:5, height:68, whiteSpace:'nowrap',
      ...
      {item.children && open && (
        <div style={{
          ...
          background:'rgba(250,250,249,0.98)', backdropFilter:'blur(16px)',
          border:'1px solid var(--light)', minWidth:160,
          boxShadow:'0 8px 32px rgba(0,0,0,0.08)', zIndex:200, padding:'8px 0',
          animation:'fadeInDown 0.18s ease',
        }}>
          {item.children.map(c => (
            <button key={c} onClick={() => go(c)} style={{
              display:'block', width:'100%', textAlign:'left', background:'none', border:'none',
              cursor:'pointer', fontFamily:'var(--font-body)', fontSize:13, color:'var(--dark)',
              padding:'11px 22px', letterSpacing:'0.04em', transition:'background 0.15s',
            }}
            onMouseEnter={e => { e.target.style.background='var(--off-white)'; e.target.style.color='var(--black)'; }}
            onMouseLeave={e => { e.target.style.background='none'; e.target.style.color='var(--dark)'; }}
            >{c}</button>
          ))}
        </div>
      )}
...
// ─── NAV ─────────────────────────────────────────────────────────────────────
function Nav({ setPage, cartCount, user, setUser, page }) {
  ...
  return (
    <div style={{ borderBottom:'1px solid var(--light)' }}>
      <button onClick={() => item.children ? setOpen(o=>!o) : go(
        ...
        width:'100%', background:'none', border:'none', cursor:'pointer', display:'flex', justifyContent:'space-between', alignItems:'center',
        padding:'18px 0', fontSize:15, color:'var(--dark)', fontFamily:'var(--font-body)', letterSpacing:'0.04em',
      }}>
        {item.label}
...
function TopProductsCarousel({ products, user, onAdd, onSelect, isMobile, setPage, promotions = [] }) {
...
function KnowledgeJournal({ isMobile }) {
    ...
    <section style={{ background:'var(--white)', padding: isMobile ? '64px 0 72px' : '104px 0 112px', borderTop:'1px solid var(--light)' }}>
      <div style={{ maxWidth:1280, margin:'0 auto', padding:'0 24px' }} className="px-page">
        <div style={{ display:'flex', justifyContent:'space-between', gap:24, alignItems:'flex-end', flexWrap:'wrap', marginBottom: isMobile ? 28 : 42 }}>
          ...
          <span style={{ fontSize:12, color:'var(--mid)', letterSpacing:'0.08em' }}>{JOURNAL_ARTICLES.length} 篇主題內容</span>
        </div>

        {isMobile ? (
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            {JOURNAL_ARTICLES.map(article => (
              <article key={article.title} style={{ border:'1px solid var(--light)', background:'var(--off-white)', display:'flex', flexDirection:'column' }}>
          ...
          <>
            <div style={{ display:'grid', gridTemplateColumns:'minmax(360px, 1.05fr) minmax(0, 1fr)', gap:24, alignItems:'stretch', marginBottom:24 }}>
              <article style={{ border:'1px solid var(--light)', background:'var(--off-white)', display:'grid', gridTemplateRows:'auto 1fr' }}>
              ...
              <div style={{ display:'grid', gridTemplateRows:'repeat(2, minmax(0, 1fr))', gap:18 }}>
                {supportingArticles.slice(0, 2).map(article => (
                  <article key={article.title} style={{ border:'1px solid var(--light)', background:'var(--white)', display:'grid', gridTemplateColumns:'180px minmax(0, 1fr)' }}>
            ...
            <div style={{ overflowX:'auto', paddingBottom:4 }}>
              <div style={{ display:'grid', gridAutoFlow:'column', gridAutoColumns:'minmax(280px, 1fr)', gap:18, minWidth: supportingArticles.length > 3 ? 1180 : 'auto' }}>
                {supportingArticles.slice(2).map(article => (
                  <article key={article.title} style={{ border:'1px solid var(--light)', background:'var(--off-white)', minHeight:300, display:'flex', flexDirection:'column' }}>
...
// ─── HOME PAGE ────────────────────────────────────────────────────────────────
function HomePage({ setPage, user, cart, setCart, promotions = [], products = PRODUCTS, salesStats = emptySalesStats() }) {
...
// ─── PRODUCT DETAIL PAGE ──────────────────────────────────────────────────────
function ProductDetail({ product, user, onAdd, onBack, promotions = [] }) {
...
// ─── PRODUCT CARD ─────────────────────────────────────────────────────────────
function ProductCard({ product, user, onAdd, onSelect, promotions = [] }) {
...
function PromoSection({ promo, user, addToCart, onSelect, isMobile, promotions = [], products = PRODUCTS }) {
...
// ─── SHOP PAGE ────────────────────────────────────────────────────────────────
function ShopPage({ user, cart, setCart, promotions = [], products = PRODUCTS, salesStats = emptySalesStats() }) {
...
function CheckoutField({ label, name, type, required, placeholder, value, onChange }) {
  return (
    <div>
      ...
      <input type={type||'text'} value={value} onChange={onChange} required={required !== false} placeholder={placeholder||''}
...
// ─── CHECKOUT PAGE ────────────────────────────────────────────────────────────
function CheckoutPage({ cart, setCart, setPage, user, promotions = [] }) {
        ...
        {STEPS.map((s, i) => (
          <React.Fragment key={s}>
            <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:6 }}>
              <div style={{ width:28, height:28, borderRadius:'50%', background: step >= i + 1 ? 'var(--black)' : 'var(--light)', color: step >= i + 1 ? 'var(--white)' : 'var(--mid)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:500 }}>
                {step > i + 1 ? '✓' : i + 1}
              </div>
              <span style={{ fontSize:10, letterSpacing:'0.08em', color: step === i + 1 ? 'var(--black)' : 'var(--mid)', whiteSpace:'nowrap' }}>{s}</span>
            </div>
            {i < STEPS.length - 1 && <div style={{ flex:1, height:1, background: step > i + 1 ? 'var(--black)' : 'var(--light)', margin:'0 8px', marginBottom:18 }} />}
          </React.Fragment>
        ))}
      </div>
    );
  ...
  function OrderSummary() {
    const summary = paymentSummary || { subtotal, discount, finalSubtotal, promotion, shipping, total };
    const summaryItems = paymentSnapshot?.items || cart;
    const hasPreorderItem = summaryItems.some(item => getFulfillmentInfo(item).type === 'preorder');
    return (
  ...
  async function handleNext(e) {
    ...
    try {
      ...
      timeout = setTimeout(() => controller.abort(), PAYMENT_REQUEST_TIMEOUT_MS);
  ...
  function PaymentInfo() {
    ...
    const resp = paymentResult.response || {};
    const atmNo = resp?.ATMParam?.AtmPayNo || '';
      ...
      <div style={{ background:'var(--off-white)', padding:'24px 28px', marginBottom:20 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'auto 1fr', gap:'8px 20px', fontSize:13 }}>
          ...
          {resp?.TSNo && <>
            ...
            <span style={{ color:'var(--black)', letterSpacing:'0.08em', fontFamily:'var(--font-display)', fontVariantNumeric:'tabular-nums' }}>{resp.TSNo}</span>
          </>}
          {atmNo && <>
        ...
        {resp?.Description && <p style={{ fontSize:12, color:'var(--dark)', marginTop:12, lineHeight:1.7 }}>{resp.Description}</p>}
        ...
        {atmNo && (
          <div style={{ display:'flex', gap:10, marginTop:16, flexWrap:'wrap' }}>
            <button type="button" onClick={() => copyAtmNo(atmNo)} style={{ background:'var(--black)', border:'1px solid var(--black)', color:'var(--white)', padding:'11px 16px', fontSize:12, letterSpacing:'0.12em', cursor:'pointer', fontFamily:'var(--font-body)' }}>
              複製虛擬帳號
            </button>
        ...
        {!atmNo && paymentLink && (
          <div style={{ display:'flex', gap:10, marginTop:16, flexWrap:'wrap' }}>
            <a href={paymentLink} target="_blank" rel="noreferrer" style={{ display:'inline-flex', alignItems:'center', justifyContent:'center', padding:'11px 16px', border:'1px solid var(--black)', color:'var(--black)', textDecoration:'none', fontSize:12, letterSpacing:'0.12em', fontFamily:'var(--font-body)' }}>
              前往付款頁
            </a>
            <button type="button" onClick={() => navigator.clipboard?.writeText(paymentLink)} style={{ background:'none', border:'1px solid var(--light)', color:'var(--dark)', padding:'11px 16px', fontSize:12, letterSpacing:'0.12em', cursor:'pointer', fontFamily:'var(--font-body)' }}>
              複製付款連結
            </button>
          </div>
        )}
...
// ─── LOGIN PAGE ───────────────────────────────────────────────────────────────
const ECLADO_SITE_ORIGIN = 'https://www.ecladotaiwan.com';
...
function isEmailVerificationCallback() {
...
function getLineLoginErrorMessage() {
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex' }}>
      ...
      <div style={{ width:'100%', maxWidth:480, display:'flex', flexDirection:'column', justifyContent:'center', padding:'60px 32px', background:'var(--white)', overflowY:'auto' }}>
        ...
        {/* 登入表單 */}
        {view === 'login' && (
          <form onSubmit={handleSignIn} style={{ display:'flex', flexDirection:'column', gap:22 }}>
            <div>
              <label style={{ fontSize:11, letterSpacing:'0.14em', color:'var(--dark)', textTransform:'uppercase', display:'block', marginBottom:9 }}>Email</label>
            ...
            <div>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:9 }}>
                ...
                <button type="button" onClick={() => { setView('forgot'); reset(); }} style={{ background:'none', border:'none', cursor:'pointer', fontSize:12, color:'var(--dark)', fontFamily:'var(--font-body)', letterSpacing:'0.04em', textDecoration:'underline', textUnderlineOffset:3 }}>忘記密碼？</button>
              </div>
              <input type="password" value={password} onChange={e=>setPassword(e.target.value)} required style={inputStyle}
                onFocus={e=>e.target.style.borderBottomColor='var(--dark)'}
                onBlur={e=>e.target.style.borderBottomColor='var(--light)'} />
            </div>
            <button type="submit" disabled={loading} style={{ width:'100%', background: loading ? 'var(--mid)' : 'var(--black)', color:'var(--white)', border:'none', padding:'15px', fontSize:12, letterSpacing:'0.2em', textTransform:'uppercase', cursor: loading ? 'default' : 'pointer', fontFamily:'var(--font-body)', fontWeight:500, marginTop:4 }}>
              {loading ? '登入中...' : '登入'}
            </button>
          </form>
        )}
        ...
        {view === 'register' && (
          <form onSubmit={handleRegister} style={{ display:'flex', flexDirection:'column', gap:22 }}>
            {/* 基本欄位 */}
            <div>
              <label style={{ fontSize:11, letterSpacing:'0.14em', color:'var(--dark)', textTransform:'uppercase', display:'block', marginBottom:9 }}>姓名</label>
              <input type="text" value={name} onChange={e=>setName(e.target.value)} required style={inputStyle}
                onFocus={e=>e.target.style.borderBottomColor='var(--dark)'}
                onBlur={e=>e.target.style.borderBottomColor='var(--light)'} />
            </div>
            <div>
              <label style={{ fontSize:11, letterSpacing:'0.14em', color:'var(--dark)', textTransform:'uppercase', display:'block', marginBottom:9 }}>手機</label>
              <input type="tel" value={phone} onChange={e=>setPhone(e.target.value)} required placeholder="09xx-xxx-xxx" style={inputStyle}
                onFocus={e=>e.target.style.borderBottomColor='var(--dark)'}
                onBlur={e=>e.target.style.borderBottomColor='var(--light)'} />
            </div>
            <div>
        ...
        {/* 忘記密碼表單 */}
        {view === 'forgot' && (
          <form onSubmit={handleForgot} style={{ display:'flex', flexDirection:'column', gap:22 }}>
            <div>
              <label style={{ fontSize:11, letterSpacing:'0.14em', color:'var(--dark)', textTransform:'uppercase', display:'block', marginBottom:9 }}>Email</label>
              <input type="email" value={email} onChange={e=>setEmail(e.target.value)} required style={inputStyle}
                onFocus={e=>e.target.style.borderBottomColor='var(--dark)'}
        ...
        {view !== 'forgot' && (
          <>
            ...
            <button onClick={signInWithLine} style={{ width:'100%', background:'#00B900', color:'var(--white)', border:'none', padding:'14px', fontSize:13, letterSpacing:'0.08em', cursor:'pointer', fontFamily:'var(--font-body)', fontWeight:500, display:'flex', alignItems:'center', justifyContent:'center', gap:10 }}>
              <svg width="20" height="20" viewBox="0 0 20 20" fill="white"><path d="M10 2C5.6 2 2 5.1 2 8.8c0 2.5 1.6 4.7 4 5.9l-.5 2 2.3-1.2c.7.2 1.4.3 2.2.3 4.4 0 8-3.1 8-6.8C18 5.1 14.4 2 10 2z"/></svg>
              使用 LINE 帳號登入
            </button>
          </>
    ...
    <div style={{ minHeight:'100vh', display:'flex' }}>
      <div className="login-visual" style={{ flex:1, display:'flex', flexDirection:'column', justifyContent:'center', alignItems:'center', background:'var(--black)', padding:60, position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ position:'relative', zIndex:1, textAlign:'center' }}>
          <div style={{ fontFamily:'var(--font-display)', fontSize:26, letterSpacing:'0.22em', color:'var(--white)', marginBottom:6 }}>ECLADO</div>
          <div style={{ fontSize:9, letterSpacing:'0.3em', color:'var(--gold)', marginBottom:40, textTransform:'uppercase' }}>Korean Cosmeceuticals</div>
          <p style={{ fontFamily:'var(--font-display)', fontSize:30, fontWeight:300, color:'var(--white)', lineHeight:1.4, marginBottom:20 }}>Reset<br />Password.</p>
...
function InfoPage() {
  ...
  const sections = {
    ...
    '常見問題': { title:'常見問題', content:[
      { q:'如何成為美容師會員？', a:'請先完成一般會員註冊並登入，再前往「美容師申請」頁面填寫資料送出。審核通過後即可享有院線商品與專業折扣。' },
...
function PromoDiagnosticBar({ status, errorText }) {
  if (status === 'loading') {
    return (
...
function App() {
  ...
  const [products, setProducts] = useState(() => PRODUCTS.map(product => ({ ...product, stock: null })));
  ...
  // 載入進行中的活動（含 realtime 同步）
  useEffect(() => {
    if (!window.supabase || typeof window.supabase.from !== 'function') {
      setPromoFetchStatus('error');
      setPromoFetchError('Supabase 未初始化（請重新整理或檢查 script 是否被擋）');
      setPromotions([]);
      return;
    }
    const sb = window.supabase;
    ...
    async function loadPromotions() {
      setPromoFetchStatus('loading');
      setPromoFetchError('');
      let { data, error } = await sb.from('promotions').select('*').order('created_at', { ascending: false });
      // 若資料表沒有 created_at 或排序失敗，改不排序再試一次
      if (error) {
        const r2 = await sb.from('promotions').select('*');
        data = r2.data;
        error = r2.error;
      }
      if (error) {
        ...
        setPromotions([]);
        setPromoFetchStatus('error');
        setPromoFetchError(error.message || String(error.code || ''));
        return;
      }
      setPromotions(data || []);
      setPromoFetchStatus('ok');
    }
    loadPromotions();
    let ch = null;
    try {
      ...
      console.warn('[ECLADO] Realtime 訂閱失敗（不影響讀取）', e);
    }
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('[ECLADO] products Realtime 訂閱失敗（不影響讀取）', e);
    }
