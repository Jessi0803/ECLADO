...
function DiscountPromotionForm({ promo, products, scopes, onClose }) {
  ...
  const giftVariants = useMemo(() => products.filter(product => product.publicationStatus === 'gift_only').flatMap(product => (product.variants || []).filter(variant => variant.active !== false).map(variant => ({ ...variant, productName:product.nameZh }))), [products]);
