...
export default function CheckoutField({ label, hint, name, type, required, placeholder, value, onChange, inputMode, maxLength }) {
  const hintId = hint ? `${name}-hint` : undefined;
  return (
    <div>
      <label htmlFor={`checkout-${name}`} style={{ fontSize:11, letterSpacing:'0.12em', color:'var(--dark)', textTransform:'uppercase', display:'block', marginBottom: hint ? 4 : 7, fontFamily:'var(--font-body)' }}>
      ...
      <input id={`checkout-${name}`} type={type||'text'} name={name} value={value} onChange={onChange} required={required !== false} placeholder={placeholder||''} aria-describedby={hintId} inputMode={inputMode} maxLength={maxLength}
